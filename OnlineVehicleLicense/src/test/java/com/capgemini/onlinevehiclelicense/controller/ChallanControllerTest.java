...
class ChallanControllerTest {
	...
	private Page<Challan> challanList;
	private List<Challan> challanByNumberList;

	@BeforeEach
	void setUp() throws Exception {
		
		this.challanList.and(new Challan("21-09787", "MH567L76", 500.00, "PAID"));
		this.challanList.and(new Challan("21-67853", "MH567L76", 250.00, "PAID"));
		this.challanList.and(new Challan("21-98798", "MH153C09", 1000.00, "NOT PAID"));
	}
	 ...
	 */
	@Test
	void testViewAllChallan(Pageable pageable) throws Exception {
		given(challanService.viewAllChallanDetails(pageable)).willReturn(this.challanList);
		
		this.mockMvc.perform(get("/api/v1/challan/view-all-challan"))
		.andExpect(status().isOk())
		.andExpect(jsonPath("$.size()", is(challanList.getSize())));
	}
	...
	void testViewChallanDetailsByVehicleNumber() throws Exception {
		...
		given(challanService.getChallanByVehicleNumber(vehicleNumber)).willReturn(this.challanByNumberList);
